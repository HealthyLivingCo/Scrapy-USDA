...
class USDASpider(Spider):
	...
	def __init__(self):
		...
		for location in locations:
			self.start_urls.append(url_pattern.format(offsetnum=location, apikey=""))
	...
	def parse(self, response):
		...
		for object in jsonresponse["report"]["foods"]:
			...
			'''
			for fooditem in object["nutrients"]:
				item["nutrient_id"] = fooditem["nutrient_id"]
				item["nutrient"] = fooditem["nutrient"]
				item["unit"] = fooditem["unit"]
				item["value"] = fooditem["value"]
				item["gm"] = fooditem["gm"]
			'''
			items.append(item)
		return items